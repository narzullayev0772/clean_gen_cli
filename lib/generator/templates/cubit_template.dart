...
class CubitTemplate {
  static String generate(FeatureSchema schema) {
    ...
    final fetcherImport =
        schema.globalConfig.config['fetcher_import'] ??
        '../../../../core/utils/fetcher.dart';
...
''';
  ...
  static String _generateDependencies(List<FunctionDef> functions) {
    return functions
        .map(
          (f) =>
              "  final ${FileWriter.toCamelCase(f.name)}UseCase _${f.name[0].toLowerCase()}${f.name.substring(1)}UseCase;",
        )
        .join('\n');
  }
  ...
  static String _generateConstructor(
    ...
    final params = functions
        .map(
          (f) =>
              "this._${f.name[0].toLowerCase()}${f.name.substring(1)}UseCase,",
        )
        .join('\n    ');
  ...
  static String _generateMethods(
    ...
    return functions
        .map((f) {
          final useCaseVar =
              '_${f.name[0].toLowerCase()}${f.name.substring(1)}UseCase';
...
class CubitStateTemplate {
  static String generate(FeatureSchema schema) {
    ...
    final constructorParams = functions
        .map((f) => '    required this.${f.name}State,')
        .join('\n');

    final copyWithParams = functions
        .map((f) {
          final responseType = ModelGenerator.getResponseModelType(f);
          final baseResponseType = ModelGenerator.getBaseResponseModelType(f);
          final type = f.pagination
    ...
    final copyWithAssignments = functions
        .map(
          (f) => '      ${f.name}State: ${f.name}State ?? this.${f.name}State,',
        )
        .join('\n');

    final initialFields = functions
        .map((f) {
          final responseType = ModelGenerator.getResponseModelType(f);
          final baseResponseType = ModelGenerator.getBaseResponseModelType(f);
          if (f.pagination) {
